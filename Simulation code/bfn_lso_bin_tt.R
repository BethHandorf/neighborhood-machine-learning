...
for (j in 1:m) {
  ...
  for (i in 1:1000) {
    mod<-glm(y[set.ind==0] ~ XCEN[set.ind==0,][, i],family=binomial)
    sim.coef[i]<-summary(mod)$coef[2,1]
    sim.pval[i]<-summary(mod)$coef[2,4]
    
    mod<-glm(y[set.ind==1] ~ XCEN[set.ind==1,][, i],family=binomial)
  ...
  cvfit = cv.glmnet(XCEN[set.ind==0,], y[set.ind==0],alpha=1,family="binomial")
...
write.table(BFN.res,"BFN_res_bin.csv", col.names =FALSE,append=TRUE, sep = ",", row.names = FALSE,quote=FALSE)
write.table(LSO.1se.res,"LSO_1se_res_bin.csv", col.names =FALSE,append=TRUE, sep = ",", row.names = FALSE,quote=FALSE)
write.table(LSO.min.res,"LSO_min_res_bin.csv", col.names =FALSE,append=TRUE, sep = ",", row.names = FALSE,quote=FALSE)
